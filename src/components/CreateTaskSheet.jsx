...
const INITIAL = { title: '', notes: '', tag_id: '', is_urgent: false, assigned_to: '' };

export default function CreateTaskSheet({ isOpen, onClose, onSubmit, onSave, tags, currentUser, userList, onCreateTag, parentTask, editTask }) {
  ...
  useEffect(() => {
    ...
    if (editTask) {
      setForm({
        ...
        tag_id: editTask.tag_id || '',
        is_urgent: editTask.is_urgent || false,
        assigned_to: editTask.assigned_to || '',
      });
    } else {
      setForm({ ...INITIAL, tag_id: parentTask?.tag_id || '', assigned_to: currentUser || '' });
    }
  }, [isOpen, editTask, parentTask, currentUser]);

  const doSubmit = () => {
    if (!form.title.trim()) return;
    if (isEdit) {
      onSave({ ...editTask, ...form });
    } else {
      onSubmit({ ...form, parent_id: parentTask?.id || null, created_by: currentUser });
    }
  };
  ...
  const sheetTitle = isEdit ? 'Aufgabe bearbeiten' : (parentTask ? 'Follow-up erstellen' : 'Neue Aufgabe');
  const buttonLabel = isEdit ? 'Speichern' : 'Aufgabe erstellen';
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 60, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(4px)' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: 430, width: '100%', background: 'white', borderRadius: '24px 24px 0 0', padding: '24px 24px 40px 24px', boxShadow: '0 -4px 24px rgba(0,0,0,0.08)', transform: `translateY(${dragY}px)`, transition: dragging ? 'none' : 'transform 0.15s ease-out', maxHeight: '85vh', overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
        ...
        {parentTask && !isEdit && (
          <p style={{ fontSize: 13, color: '#7D917E', marginBottom: 16 }}>Follow-up zu: <span style={{ fontWeight: 600, color: '#2C3E2D' }}>{parentTask.title}</span></p>
        )}

        <label style={{ fontSize: 13, fontWeight: 600, color: '#7D917E', display: 'block', marginBottom: 6 }}>Titel</label>
        <input type="text" value={form.title} onChange={e => update('title', e.target.value)} placeholder="Was ist zu tun?"
        ...
        <label style={{ fontSize: 13, fontWeight: 600, color: '#7D917E', display: 'block', marginBottom: 6 }}>Kurze Notiz</label>
        <input type="text" value={form.notes} onChange={e => update('notes', e.target.value)} placeholder="Optional"
          style={{ width: '100%', padding: '14px 16px', border: '1.5px solid #E4EBE4', borderRadius: 14, fontSize: 15, color: '#2C3E2D', background: '#FAFAF8', marginBottom: 12, outline: 'none', fontFamily: "'Plus Jakarta Sans', sans-serif" }}
          onFocus={e => { e.target.style.borderColor = '#5E8B62'; e.target.style.boxShadow = '0 0 0 3px rgba(94,139,98,0.1)'; }}
          onBlur={e => { e.target.style.borderColor = '#E4EBE4'; e.target.style.boxShadow = 'none'; }}
        />
